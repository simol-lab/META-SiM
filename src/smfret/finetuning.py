...
def finetune_classification_head(
    encoder,
    train_sets,
    epochs,
    num_classes=2,
    ...
    if trace_level:
        if embedding_list is None:
            embedding_list = [
                encode_traces(trace_set, encoder) for trace_set in train_sets
            ]
        label_list = [
            tf.cast(np.max(trace_set.label, axis=-1), tf.int64)
            for trace_set in train_sets
        ]
    else:
        if embedding_list is None:
            embedding_list = [
                tf.reshape(
                    encode_traces(trace_set, encoder), (-1, encoder.output_shape[-1])
                )
                for trace_set in train_sets
            ]
        label_list = [
            tf.cast(tf.reshape(trace_set.label, (-1,)), tf.int64)
            for trace_set in train_sets
        ]
    ...
    if use_sklearn:
        ...
        if clf.coef_.shape[0] == 1:
            ...
            clf.intercept_ = np.concatenate(
                [np.zeros_like(clf.intercept_), clf.intercept_], axis=0
            )
        num_classes = clf.coef_.shape[0]

    readout = keras.layers.Dense(
        num_classes, kernel_regularizer=keras.regularizers.l2(l2_regularization)
    )
