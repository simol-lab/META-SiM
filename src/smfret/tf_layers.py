...
class Conv(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.filter = self.add_weight(
            "weight",
            shape=[self.width, input_dim, self.inner_dim],
            initializer=tf.keras.initializers.GlorotUniform(),
        )
        self.bias = self.add_weight(
            "bias", shape=[1, 1, self.inner_dim], initializer=tf.zeros_initializer()
        )
...
class Reconstructor(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.remap = self.add_weight(
            "remap",
            shape=[input_dim, self.stride, input_dim],
...
class Summary(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.k = self.add_weight(
            "K",
            shape=[self.head_dim, input_dim],
            initializer=tf.keras.initializers.GlorotUniform(),
        )
        self.v = self.add_weight(
            "V",
            shape=[self.head_dim, input_dim, self.outer_dim],
            initializer=tf.keras.initializers.GlorotUniform(),
        )
        self.b = self.add_weight(
            "B", shape=(1, self.outer_dim), initializer=tf.zeros_initializer()
        )
    ...
    def call(self, inputs):
        ...
        w = tf.einsum("hi,bti->bht", k, inputs) / tf.sqrt(
            tf.cast(self.inner_dim, inputs.dtype)
        )
...
class Embedding(keras.layers.Layer):
    ...
    def __init__(
        self, conv, transformer, prepend_task_token, reconstructor=None, framewise=False
    ):
        super().__init__()
        self.conv = conv
    ...
    def get_visualization_token(self, seq):
        "Gets the first frame level token for projector visualization."
        local_features_input = self.conv(seq)
        seq_input = self.prepend_task_token(local_features_input)
        seq_output = self.transformer(seq_input)
        return seq_output[:, 1, :]
...
class Transformer(keras.layers.Layer):
    ...
    def __init__(
        ...
        if self.use_layer_norm:
            self.layer_norm_layers = [
                keras.layers.LayerNormalization() for _ in range(2 * self.depth)
            ]

        self.attention_layers = [
            keras.layers.MultiHeadAttention(
                num_heads=self.num_heads, key_dim=self.inner_dim
            )
            for _ in range(self.depth)
        ]
        self.attention_dropout = [
            keras.layers.Dropout(self.dropout) for _ in range(self.depth)
        ]
        ...
        for _ in range(self.depth):
            ...
            for dense_unit in [*self.dense_width_array, self.inner_dim]:
                self.dense_layers[-1].append(
                    keras.layers.Dense(
                        units=dense_unit, activation=self.dense_activation
                    )
                )
    ...
    def call(self, base_seq_input):
        ...
        for depth in range(self.depth):
            ...
            for dense_ff, dropout in zip(
                self.dense_layers[depth], self.dense_dropout[depth]
            ):
...
class PositionEmbedding(tf.keras.layers.Layer):
    ...
    def __init__(
        self, max_length=400, initializer="glorot_uniform", seq_axis=1, **kwargs
    ):
